...
feeding_data, _ = process_feeding_data(feeding_data)
...
for fold, (train_idx, test_idx) in enumerate(group_kfold.split(X, y, flt_pts)):
    ...
    # print(f"{states_test.shape=}, {actions_train.shape=}, {rewards_test.shape=}, {flt_pts_test_tensor.shape}, {y_train.shape=}, {ax_states_test.shape}")

    dataset_train = TensorDataset(states_train, actions_train, rewards_train, flt_pts_train_tensor, y_train, ax_states_train)
    dataset_test = TensorDataset(states_test, actions_test, rewards_test, flt_pts_test_tensor, y_test, ax_states_test)
    ...
    for i in range(len(insulins_administrated_test)):
        ...
        patient_feeding_insulin = {
            ...
            "pt_feeding_start_time":flt_feed_test[i]["feeding_start_time"],
            "BST_diff": y_test[i],
            ...
            "target_bst": flt_feed_test[i]["target_bst"],
            "target_bst_idx": flt_feed_test[i]["target_bst_idx"],
            "initial_bst": flt_feed_test[i]["initial_bst"],
            "initial_bst_idx": flt_feed_test[i]["initial_bst_idx"],
            "IN_upper_hr": flt_feed_test[i]["IN_upper"],
            "IN_lower_hr": flt_feed_test[i]["IN_lower"],
            "IN_delta_hr": flt_feed_test[i]["IN_delta"]
        }
        patient_feeding_insulin_info.append(patient_feeding_insulin)

    print(f"{fold} complete.")

eval_results, df = evaluate_insulin_predictions(patient_feeding_insulin_info)
# print("\n--- Evaluation Results ---")
# for group, metrics in eval_results.items():
#     if group != "overall_accuracy":
#         print(f"{group}: Accuracy = {metrics['accuracy']:.2f}% ({metrics['correct']} / {metrics['total']})")
# print(f"Overall Accuracy: {eval_results['overall_accuracy']:.2f}%")
print("\n--- Evaluation Results ---")
for group, metrics in eval_results.items():
    if group in {"Low", "Normal", "High"}:
        print(f"{group}: Accuracy = {metrics['accuracy']:.2f}% ({metrics['correct']} / {metrics['total']})")
print(f"Overall Accuracy: {eval_results['overall_accuracy']:.2f}%")
print("\n--- Accuracy by Hospital ---")
for hospital, metrics in eval_results["by_hospital"].items():
    print(f"{hospital}: Accuracy = {metrics['accuracy']:.2f}% ({metrics['correct']} / {metrics['total']})")
...
with open(os.path.join(save_path, 'patient_feeding_insulin_info.pkl'), 'wb') as f:
    pickle.dump(patient_feeding_insulin_info, f)
